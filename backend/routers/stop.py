...
def shift_block_up(db: Session, route_id: int, start_seq: int, end_seq: int) -> None:
    ...
    db.execute(                                                           # Phase 1: move block into safe zone
        ...
        .values(sequence=stop_model.Stop.sequence + SHIFT_OFFSET)         # sequence = sequence + OFFSET
        .execution_options(synchronize_session=False)                     # Do not sync ORM state (prevents collisions)
    )
    ...
    db.execute(                                                           # Phase 2: bring block back shifted +1
        ...
        .values(sequence=stop_model.Stop.sequence - SHIFT_OFFSET + 1)     # final = original + 1
        .execution_options(synchronize_session=False)                     # Do not sync ORM state (prevents collisions)
    )
    ...
    try:                                                                  # Start protected DB operation
        ...
        # -----------------------------------------------------------
            ...
            if target <= max_seq:                                         # If inserting into the middle
                shift_block_up(db, payload.route_id, target, max_seq)     # Shift [target..max] down by +1 (make room)
...
def reorder_stop(stop_id: int, payload: schemas.StopReorder, db: Session = Depends(get_db)):
    ...
    try:                                                                  # Protected transaction
        ...
        stop.sequence = stop.sequence + OFFSET                            # Phase 1: move current stop out of the way
        ...
        if new_seq < old_seq:                                             # Moving upward (e.g., 5 → 2)
            shift_block_up(db, route_id, new_seq, old_seq - 1)            # Shift [new..old-1] down by +1 (make room)
        else:                                                             # Moving downward (e.g., 2 → 5)
            shift_block_down(db, route_id, old_seq + 1, new_seq)          # Shift [old+1..new] up by -1 (fill gap)
