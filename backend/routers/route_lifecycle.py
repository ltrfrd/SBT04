...
def _raise_district_planning_path_retired() -> None:
    raise HTTPException(
        ...
        detail=(
            ...
            "Use /districts/{district_id}/routes or /districts/{district_id}/routes/{route_id}."
        ),
    )


def _create_route_internal(
    *,
    route: RouteCreate,
    db: Session,
    operator: Operator,
...
@router.delete("/{route_id}", status_code=status.HTTP_204_NO_CONTENT, include_in_schema=False)
def delete_route(
    route_id: int,
    db: Session = Depends(get_db),
    operator: Operator = Depends(get_operator_context),
