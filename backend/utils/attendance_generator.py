...
def generate_attendance(
    ...
    if attendance_type == "driver" and ref_id:
        return driver_summary(db, ref_id, operator_id=operator_id)  # Return driver attendance summary
    ...
    if attendance_type == "date" and start and end:               # Date-based attendance request
        return date_summary(db, start, end, operator_id=operator_id)                       # Generate daily attendance summary
    if attendance_type == "school" and ref_id:                   # School-based attendance request
        return school_summary(db, ref_id, operator_id=operator_id)                        # Generate school attendance summary
...
# -----------------------------------------------------------
def date_summary(db: Session, start: date, end: date, operator_id: int | None = None):        # Build attendance for a specific date
...
# -----------------------------------------------------------
def school_summary(db: Session, school_id: int, operator_id: int | None = None):  # Build attendance for a school
...
def school_routes_summary(db: Session, school_id: int, operator_id: int | None = None):
    school_data = school_summary(db, school_id, operator_id=operator_id)  # Reuse existing school attendance payload
...
def school_route_runs_summary(db: Session, school_id: int, route_id: int, operator_id: int | None = None):
    school_data = school_summary(db, school_id, operator_id=operator_id)  # Reuse existing grouped payload
